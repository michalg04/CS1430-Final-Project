...
def create_results_webpage(train_image_paths, test_image_paths,
	...
	with open('results_webpage/index.html', 'w+') as f:
		...
		true_pos =np.mean( np.diag(confusion_matrix))
		precision1 = (np.sum(true_pos / np.sum(confusion_matrix, axis=0)))/num_class
		recall1 = (np.sum(true_pos / np.sum(confusion_matrix, axis=1)))/num_class
		f1_score1 = 2* (precision1 * recall1) /(precision1+ recall1)
		
		print('Accuracy (mean of diagonal of confusion matrix) is {:2.3%}'.format(accuracy))
		print('Precision is {:2.3%}'.format(precision1))
		print('Recall is {:2.3%}'.format(recall1))
		print('F1 score is {:2.3%}'.format(f1_score1))
